...
export const consultationLead = defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      title: "手机号",
      type: "string",
      description: "[内部管理: 用户提交咨询时填写的手机号]",
      validation: (rule) => rule.required(),
    }),
    defineField({
      name: "age",
      title: "年龄",
      type: "number",
      description: "[内部管理: 用户填写的年龄]",
    }),
    defineField({
      name: "city",
      title: "所在城市",
      type: "string",
      description: "[内部管理: 用户填写的所在城市]",
    }),
    defineField({
      name: "consultationType",
      title: "咨询项目",
      type: "string",
      description: "[内部管理: 用户选择的咨询类型]",
      options: {
    ...
    defineField({
      ...
      type: "text",
      rows: 3,
      description: "[内部管理: 用户填写的补充说明]",
    }),
    defineField({
      name: "sourcePage",
      title: "来源页面",
      type: "string",
      description: "[内部管理: 自动记录用户提交咨询的页面路径]",
    }),
    defineField({
      name: "utmSource",
      title: "UTM Source",
      type: "string",
      description: "[内部管理: 自动记录流量来源，如: baidu、google]",
    }),
    defineField({
      name: "utmMedium",
      title: "UTM Medium",
      type: "string",
      description: "[内部管理: 自动记录推广渠道，如: cpc、organic]",
    }),
    defineField({
      name: "utmCampaign",
      title: "UTM Campaign",
      type: "string",
      description: "[内部管理: 自动记录推广活动名称]",
    }),
    defineField({
      name: "submittedAt",
      title: "提交时间",
      type: "datetime",
      description: "[内部管理: 自动记录咨询提交时间]",
    ...
    defineField({
      ...
      title: "跟进状态",
      type: "string",
      description: "[内部管理: 线索跟进状态，用于标记处理进度]",
      options: {
        list: STATUS_OPTIONS,
        layout: "dropdown",
      },
    ...
    defineField({
      ...
      type: "text",
      rows: 2,
      description: "[内部管理: 团队内部跟进备注，前台不展示]",
    }),
  ],
  orderings: [
    {
