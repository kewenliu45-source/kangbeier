...
export function MobileNav({ siteSettings, contactInfo }: MobileNavProps) {
  ...
  // 字段优先级
  const brandName = siteSettings?.brandName || siteSettings?.siteName || "好孕生命中心";
  const phone = contactInfo?.phone || siteSettings?.primaryPhone || "155-2728-3220";
  const wechatId = contactInfo?.wechatId || siteSettings?.primaryWechat || "15527283220";
  ...
  // 微信二维码
  const wechatQrCode = contactInfo?.wechatQrCode || siteSettings?.wechatQrCode;
      ...
      {open &&
        createPortal(
          <>
            ...
            {/* 侧边面板 */}
            <div
              className="fixed top-0 right-0 z-[101] h-screen w-[280px] max-w-[85vw] bg-white shadow-2xl transform transition-transform duration-300 ease-in-out translate-x-0"
              onTouchStart={handleTouchStart}
              onTouchEnd={handleTouchEnd}
            >
              {/* 面板头部 */}
              <div className="flex items-center justify-between px-6 h-[80px] border-b border-border">
                <Link href="/" onClick={handleClose} className="text-lg font-bold text-primary">
                  {brandName}
                </Link>
                <button
                  type="button"
              ...
              <nav className="flex-1 px-4 py-6 overflow-y-auto">
                ...
                <div className="mt-6 px-4">
                  <div className={cn("p-4 rounded-2xl", "bg-white border border-border")}>
                    <p className="text-sm font-semibold text-primary mb-1">添加微信</p>
                    <p className="text-xs text-muted-foreground mb-3">获取试管科普与咨询提醒</p>
                    {wechatQrCode ? (
                      <div className="w-full aspect-square rounded-xl overflow-hidden">
                        <Image
                          src={urlForImage(
                            wechatQrCode as unknown as Parameters<typeof urlForImage>[0]
                          )
                            .width(400)
                            .url()}
                          alt="公众号二维码"
                      ...
                      >
                        ...
                        <span className="text-xs text-muted-foreground/60">公众号二维码</span>
                      </div>
                    )}
                  </div>
                </div>
